...
public class Hunter extends Player implements ClassInterface{
    ...
    //Tworzenie klasy hunter, przypisywanie multiplikatorow oraz resetowanie statystyk (w razie zmiany klasy)
    public Hunter(){
        this.lvl = 1;
        this.exp = 0;
        this.points = 10;
        this.expBar = 30;
        this.multiplier.put("Strength multiplier", 1.05f);
        this.multiplier.put("Defence multiplier", 1.05f);
        this.multiplier.put("Intelligence multiplier", 1.00f);
        this.multiplier.put("Dexterity multiplier", 1.20f);
        this.multiplier.put("Agility multiplier", 1.10f);
    ...
    //System obrazen wraz z krytykiem!!!
    @Override
    public Float getDmg(){
        Random rand = new Random();
        Float random = rand.nextFloat();
        Float random2 = rand.nextFloat(getDexterity() / 5);
        Float percentage = (getStrength() * 0.015f);
        if(random <= percentage){
            return(float)Math.round((getDexterity() * 1.5 + getAgility() + random2) * 100)/ 100;
        }
        else {
            return (float)Math.round((getDexterity() + random2) * 100) / 100;
        }
    }
    ...
    public void addDefence(Integer points) {
        if(this.points >= points){
            ...
            setHp((float)Math.round((getHp() + (getDefence() / 2) + points)  * 100.0f) / 100.0f);
        }
    }
    ...
    //System dodawanie exp'a, oczywiscie zabezpieczony przed wykroczeniem poza maksymalny poziom
    @Override
    ...
    //Metoda prywatna, poniewaz nie chce aby ktos z niej korzystal spoza klasy (Utworzona na potrzeby addExp)
    private void lvlUp() {
        this.lvl += 1;
        this.points += 10;
    ...
    //Metoda uzywana tylko przez inne metody w tej klasie, dlatego private
